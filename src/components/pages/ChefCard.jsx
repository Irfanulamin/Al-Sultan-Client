...
const ChefCard = ({ chefData }) => {
  ...
  return (
    <div className="card card-side border bg-white border-slate-600 rounded-sm relative">
      <div className="absolute  top-5 right-2 h-16 w-16">
        <CheckBadgeIcon className="h-9 w-9 text-slate-950"></CheckBadgeIcon>
      </div>
      ...
      <div className="card-body">
        ...
        <div className="card-actions justify-end">
          <Link
            to={`chefs/${id}`}
            className="btn btn-outline btn-xs primary_text capitalize text-amber-600 border-amber-600 rounded-sm text-xs  hover:bg-amber-600/90 hover:border-amber-600 hover:font-normal"
          >
            View Recipes
          </Link>
        </div>
